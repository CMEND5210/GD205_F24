...
public class BookResourceHard : MonoBehaviour
{
    ...
    //We create a float variable, named CurrentBookAmount, which is equal to 10f.
    ...
    void Update()
    {
            ...
            //We create a for loop for our Potatoes array.
        {
            if (Potatoes[i].transform.position.x == exitPlace.transform.position.x && Potatoes[i].transform.position.z == exitPlace.transform.position.z && Potatoes[i].GetComponent<PotatoSneakHard>().Decider <= 0.5f)
                ...
                //As well as whenever the Potato's Decider float is less than or equal to 0.5f.
            {
                CurrentBookAmount -= 1.5f;
                //We subtract 1.5f from the CurrentBookAmount
                Destroy(Potatoes[i]);
